	...
	<div class="main">
      ...
      <div class="content">
            ...
            <%
            	int count=0;
            	int i=0;
            	List<Shopcur> s1=(List<Shopcur>) request.getAttribute("shopcurlist");
            %>
                <table border="1">
				<tr><th>图书名</th>
				<th>图书数量</th>
				...
				<th>从购物车移除</th>
				<th>购买更多本</th></tr>
				<c:forEach items="${shopcurlist}" var="sh">
				<tr>
				<td>
					<%
					if(s1.size()>0){
			        	...
			        	Book b=se.get(Book.class, s1.get(i).getBookId());
			        	String bookName=b.getName();
			        	tran.commit();
			        	se.close(); %>
			        	<%=bookName %> 
			        	<% 
			        	count=count+s1.get(i).getSum();
			        	i++;
					}else{
						%><% 
					}
				...
				<td>&nbsp;&nbsp;&nbsp;${sh.number}</td>
				<td>&nbsp;&nbsp;${sh.sum}</td>
				<td>&nbsp;&nbsp;<a href="deleteAshopcur.do?id=${sh.shopcurId}">删除</a></td>
				<td>&nbsp;&nbsp;<a href="Bookbuy.do?id=${sh.bookId}">购买</a></td>
				</tr>
				</c:forEach>
                </table><br/>
                <a href="deleteAllshopcur.do">清空购车</a><br/>
                总金额：￥<%=count %><a href="#">去结算</a><br/>
                <a href="jiaOrders.do">确认提交订单</a>&nbsp;&nbsp;&nbsp;
                <a href="Orderslist.do">查看订单</a><br/><br/><br/>
